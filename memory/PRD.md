...
- Iter 27 (manual product order): Admin → Produits: when ONE category chip is selected, each product row has ▲▼ (testIDs `admin-product-up/down-{id}`, disabled at ends, "#n" position in sub line); a move swaps + renumbers `sort` 1..n inside that category via PATCH /products/{id} {sort} (same pattern as categories.tsx). Pizza du mois row shows a "1er" star tag instead of arrows (always first automatically, backend + customer menu unchanged). "Tout" view is grouped by category order + shows a hint. Frontend only (`app/staff/admin/index.tsx`, i18n `reorderHint`/`alwaysFirst`). Verified: move down/up in UI, customer menu follows the order with the moment pizza first; test flag/sort reverted.

## Iteration 28 – FINAL ADMIN + CUSTOMER DATABASE PASS (testing agent iter 17: 29 backend pass; UI flows verified in browser)
- ROOT CAUSES of "Administration cannot save / Upload error": (a) staff JWT expires after 16 h – the admin kept displaying the menu (public GET /menu) while every write returned 401 with only a toast → now any 401 on a staff request LOCKS the session and shows the reason on the PIN screen (`setStaffUnauthorizedHandler` in api.ts / staff-auth.tsx); (b) photo upload sent the ORIGINAL phone photo (5–13 MB) – gateway limits/timeouts return non-JSON errors and HTTP/2 has no statusText → message was empty → "Upload error". Now photos are downscaled/re-encoded on the device (expo-image-manipulator, ≤1600 px JPEG, adaptive until < 900 KB, HEIC/PNG → JPEG) and all errors are readable (`toApiError`, HTTP 413 hint); (c) extras editor parsed prices on every keystroke → decimals ("2.50") could not be typed → now text drafts parsed on Save; (d) a NEW supplement was not in any pizza's allowed_extra_ids → invisible on the customer side → backend POST /extras now $addToSet on all customizable products and makes the key unique (jambon_2…); PUT /extras never changes the key; (e) sizes were a free-text "32 | 32cm | 18" field → structured size rows (+32/40/50, +Bambino 26cm, + autre taille), base price optional (= cheapest size), clear validation messages, new customizable products allow all supplements by default, back-navigation fallback after save.
- Clients (`/staff/customers` → `/staff/customers/[key]`, backend `customers.py`): one profile per phone identity aggregated read-only from users + orders (accounts ★ COMPTE / CLIENT TÉLÉPHONE / INVITÉ), search name/phone/e-mail, filter marketing Oui/Non, stats (orders, last order, total spent excl. cancelled), addresses used, full order history (#, date/time, RETRAIT/LIVRAISON, source, status, total → tap opens /staff/ticket/[id]). CSV export of opted-in customers (manager, header download icon).
- Marketing consent: optional checkbox (unchecked) at checkout (`marketing-consent-checkbox`) and account registration (`auth-marketing-checkbox`), account profile switch to withdraw/opt-in (`PUT /auth/me/marketing`), manager switch in the profile (`PUT /customers/{key}/marketing`, source "staff"). Stored in `marketing_consents` {phone digits, consent, at, source checkout|account|staff, history}; orders also carry `marketing_consent` (audit). Only an explicit tick ever sets consent=true; no existing customer opted in; no effect on checkout/e-mails/review flow.
- Test data (ZZTEST products/extras, TEST_ orders/users/consents) deleted; Margherita/Champignons/Olives/Ananas restored; PrintNode key restored (LIVE). Phone-role PIN 3456 no longer valid (changed by the restaurant).
- Iter 26 (Pizza du mois first – bug): ROOT CAUSE: admin editor saves via PATCH /products/{id}, which lacked the "only one moment" rule → two pizzas flagged / stale flags; also frontend only pulled moment products from pizza+custom categories. FIX: PATCH enforces single 'moment'; customer Pizzas section forces ANY moment product first (then Créer votre pizza, then rest) and hides it from its own other category. Testing agent iter 16 PASS (FR/DE, 1280/390, dynamic switch). All flags reset to none – restaurant flags its own pizza in Admin.

## Deployment readiness (health check, 2026-09-24) – PASS with warnings
- Fixed blockers: startup seed no longer deletes categories (legacy gluten_free filter category is only set `active: False`); backend/.env `EMAIL_FROM_NAME` quoted; root .gitignore no longer excludes `.env` files.
- Remaining warnings (not blocking): customers.py builds profiles in memory from all orders (fine at restaurant scale – ~1.5k orders; revisit with aggregation if it grows >50k); Apple review usually requires an in-app "delete my account" flow for customer accounts (not built yet – needs user decision).
- Security audit (2026-09-24): CRITICAL default manager PIN 1234 must be changed in Admin → Sécurité before launch; HIGH: guest order lookup GET /api/orders/{id} is public by id (proposed fix: opaque per-order token); MEDIUM: CSV formula escaping in marketing export; secrets rotation recommended before launch. None fixed yet – awaiting user decision.
