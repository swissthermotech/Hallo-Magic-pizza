...
/** Manager: end-of-day closing per driver number (Livreur 1/2/3) + daily totals per order source. */
...
function DriverBlock({ d }: { d: DriverClosing }) {
  ...
  return (
    <View style={styles.card} testID={`closing-${d.driver.replace(" ", "-")}`}>
      ...
      <View style={styles.divider} />
      <View style={styles.row}><Text style={styles.total}>{t("total")}</Text><Text style={styles.total} testID={`closing-total-${d.driver.slice(-1)}`}>{chf(d.total)}</Text></View>

      <View style={styles.inputs}>
        <View style={{ flex: 1, gap: 4 }}>
          <Text style={styles.small}>{t("actualCash")}</Text>
          <TextInput testID={`closing-actual-cash-${d.driver.slice(-1)}`} value={cash} onChangeText={setCash} keyboardType="decimal-pad" placeholder="0.00" placeholderTextColor={colors.muted} style={styles.input} />
        ...
        <View style={{ flex: 1, gap: 4 }}>
          ...
          <TextInput testID={`closing-actual-terminal-${d.driver.slice(-1)}`} value={term} onChangeText={setTerm} keyboardType="decimal-pad" placeholder="0.00" placeholderTextColor={colors.muted} style={styles.input} />
          <View style={styles.row}><Text style={styles.small}>{t("difference")}</Text><Diff v={diff(term, d.terminal_expected)} /></View>
        </View>
      </View>
      <Button title={t("saveClosing")} icon="save" variant="secondary" loading={save.isPending} onPress={() => save.mutateAsync({ date: d.date, driver: d.driver, actual_cash: num(cash), actual_terminal: num(term) }).then(() => toast.show(t("saved"), "success")).catch((e) => toast.show(e.message, "error"))} testID={`closing-save-${d.driver.slice(-1)}`} />
      {d.closed_at ? <Text style={styles.small}>✓ {t("saved")} {fmtTime(d.closed_at)}</Text> : null}

      {d.orders.length ? (
        <Pressable testID={`closing-toggle-${d.driver.slice(-1)}`} onPress={() => setOpen((v) => !v)} style={styles.row}><Text style={styles.link}>{open ? "−" : "+"} {d.orders.length} {t("orders").toLowerCase()}</Text></Pressable>
      ) : null}
      {open ? d.orders.map((o) => (
